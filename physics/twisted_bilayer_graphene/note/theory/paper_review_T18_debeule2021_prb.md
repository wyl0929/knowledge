 ...
 * @FilePath     : /.agents/home/wyl/project/knowledge/physics/twisted_bilayer_graphene/note/theory/paper_review_T18_debeule2021_prb.md
 * @Description  :
-->
<!--
 * @Author       : AI精读 (research_onboarding_skill, Phase 4)
 * @Date         : 2026-08-06
 * @FilePath     : /project/knowledge/physics/twisted_bilayer_graphene/note/theory/paper_review_T18_debeule2021_prb.md
...
- 将 2D 三角网络映射为等效 1D **Floquet（周期驱动）**问题——利用摩尔平移对称性
- 畴壁手征模式在 AB/BA 区域交替传播 → 等效于周期性驱动 📜Q（原文 "Floquet" 57 处；⚠️ 旧版 "时间周期的踢（kick）" 表述已弃——grep "kick"=0）
...
2D moire 网络映射为有效 1D Floquet 问题：手征模在交替畴区运动视为**周期驱动**（⚠️ 旧版 "周期踢动" 已弃，原文未用 kick 语言）→ Floquet 能带和拓扑边缘态。无需完整 2D 网络计算。
...
- **§2/Q2 "周期踢动" → "周期驱动（Floquet）"**: 原文为网络映射到周期驱动 Floquet 系统（grep "Floquet"=57、"kick"=0），"踢（kick）" 系 AI 自由翻译。依据: §B.5#15 + cards.md T18。
