 ...
 * @FilePath     : /.agents/home/wyl/project/knowledge/physics/twisted_bilayer_graphene/note/theory/paper_review_T12_tsim2020_prb.md
 * @Description  :
-->
<!--
 * @Author       : AI精读 (research_onboarding_skill, Phase 4)
 * @Date         : 2026-08-06
 * @FilePath     : /project/knowledge/physics/twisted_bilayer_graphene/note/theory/paper_review_T12_tsim2020_prb.md
...
在每个 AA 节点，散射是**确定性（置换型）**的（而非 Efimkin-MacDonald 的满矩阵散射）：

$$S_{\text{TZM}} = \begin{pmatrix} 0 & 1 & 0 \\ 0 & 0 & 1 \\ 1 & 0 & 0 \end{pmatrix} \quad \text{或等价排列}$$

即：mode 1 恒散射到 mode 2（$-120°$ 方向）、mode 2 恒散射到 mode 1（$+120°$ 方向）📜Q → 三组独立 zigzag 传播模、无分束、无杂化 → 完美 1D 传播。

> ⚠️ 2026-08-20 修正（§B.5#13）: 旧版称 "$S$ 矩阵是对角的"——原文并非对角矩阵，而是**置换型**确定性散射（原文: "mode 1 is always scattered to mode 2 in the −120° direction, while mode 2 is always scattered to mode 1 in the +120° direction… three independent zigzag traveling [modes]"）。
...
| TZM 方向数 | 3（沿三组平行畴壁方向）|
| 每方向通道数 | 1 条 TZM / 谷 / 自旋 |
| $S$ 矩阵 | 置换型确定性散射（mode 1↔mode 2，±120°；无分束不杂化）|
...
推翻螺旋网络图景：偏置 mTBG 拓扑通道不形成渗流网络，而是三组独立完美 1D 手征 zigzag 模 (TZM)，沿三个方向独立传播、永不杂化。

### Q3: 理论如何与实验结合?
mTBG 中 AB/FP 振荡可来源于 1D 通道间干涉——区分"网络输运"还是"独立 1D 链输运"的核心概念裁决。

---

## 更正日志 (2026-08-20, S-REV)

- **§3.2/§8 "S 矩阵对角"**: 改为置换型确定性散射——原文非对角矩阵，而是 "mode 1 is always scattered to mode 2 in the −120° direction, while mode 2 is always scattered to mode 1 in the +120° direction"（grep "120" ✓），形成三组独立 zigzag 模。依据: §B.5#13 + cards.md T12。
