 ...
 * @FilePath     : /.agents/home/wyl/project/knowledge/physics/twisted_bilayer_graphene/note/theory/paper_review_T09_efimkin2018_prb.md
 * @Description  :
-->
<!--
 * @Author       : AI精读 (research_onboarding_skill, Phase 4)
 * @Date         : 2026-08-06
 * @FilePath     : /project/knowledge/physics/twisted_bilayer_graphene/note/theory/paper_review_T09_efimkin2018_prb.md
...
网络能带能标 $\varepsilon_L = 2\pi\hbar v/L$（$\theta=0.245°$、$L\approx58$ nm 时 ≈72 meV 📜Q）。$\rho(E)$ 在能量上**周期性振荡**，DoS 周期为能带能标的 1/3（$D=\varepsilon_L/3$ ≈20 meV 📜Q）：
- 每周一个 Van Hove 奇点（发散）
- 每周一个 DoS 零点（Dirac 点处）

→ 实验 signature：STM $dI/dV$ 在畴壁上应出现多峰结构，峰值间距 $\propto 1/L \propto \theta$

> ⚠️ 2026-08-20 修正（§B.5#1）: 旧版把 DoS 周期写成 "$\sim L = 2\pi\hbar v/L$" 系符号混乱——能带能标与 DoS 周期是两个量（后者为前者的 1/3）。卡片旧版 "$E_\lambda = 2\pi\hbar v/3\lambda$" 同源错误，已改。
...
| 网络能带能标 | $\varepsilon_L = 2\pi\hbar v/L$（θ=0.245° 时 ≈72 meV 📜Q）|
| DoS 周期 | $D = \varepsilon_L/3$（≈20 meV 📜Q）|
...
- **§4.2/§8 能标公式**: 旧版 "DoS 周期 $\sim 2\pi\hbar v/L$" 符号混乱 → 改为能带能标 $\varepsilon_L=2\pi\hbar v/L$（≈72 meV@0.245°，grep "72 meV"/"58 nm" ✓）与 DoS 周期 $D=\varepsilon_L/3$（≈20 meV）两个量。依据: §B.5#1。
