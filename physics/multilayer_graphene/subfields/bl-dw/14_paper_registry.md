...
| **精读** | 📄 [精读报告](paper_review_li2016_natnanotech.md) |
...
| **类型** | 实验 — 输运 |
| **Zotero Key** | `ZT_KEY_NEEDED` |
| **精读** | 📄 [精读报告](paper_review_mania2019_communphys.md) |
...
## 补充：CVD 外延双层石墨烯 LSW 束输运

> CVD（化学气相沉积）外延生长的双层石墨烯中天然形成 LSW。与机械剥离样品不同，CVD 样品的 LSW 密度高、可由应变工程控制构型（分离 vs 紧密堆积），为研究 QVH 多通道耦合提供了新平台。

| # | 文献 | 贡献 | 精读 |
|---|---|---|---|
| [DW-c1] | **★ Zhang et al. (2024)** arXiv:2406.06867 | CVD 双层石墨烯中两种 LSW 构型（分离型 vs 紧密堆积束）的比较输运研究；发现紧密堆积 LSW 束展现电场可调的磁电导——栅压连续调节 QVH 通道间耦合 | 📄 [精读报告](paper_review_zhang2024_arxiv.md) |

**CVD LSW 平台的关键差异**：
- CVD LSW（DW-c1）：外延生长自组织形成，高密度 + 应变可控 + 可扩展——面向"可编程拓扑电子学"
- 非转角畴壁（P1–P7）：机械剥离随机边界，低密度不可控——面向基础物理验证
- mTBG 畴壁（DW-m1–m4）：摩尔超晶格周期性网络，转角联合调控——面向干涉/编织物理
